...
//PERSISTENT SEQUENCE
BehaviorState Elite::BehaviorPersistentSequence::Execute(Blackboard* pBlackboard)
{
	for (auto child : m_ChildrenBehaviors)
	{
		m_CurrentState = child->Execute(pBlackboard);
		switch (m_CurrentState)
		{
			//Continue even after Failure has occured
		case Elite::Failure:
		case Elite::Success:
			continue; break;
			break;
...
BehaviorState BehaviorConditional::Execute(Blackboard* pBlackBoard)
{
	...
	bool result = m_fpConditional(pBlackBoard);
	if (m_InvertCondition) result = !result;
